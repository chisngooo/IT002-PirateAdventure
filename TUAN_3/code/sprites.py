...
class Item(AnimatedSprite):
    '''
    Khởi tạo lớp Item kế thừa từ lớp AnimatedSprite
    Lớp này tạo ra các đối tượng đồ họa là Item trong trò chơi 
    '''
    def __init__(self, item_type, pos, frames, groups,data):
        super().__init__(pos, frames, groups)
        self.rect.center = pos # đặt vị trí của item ở giữa
        self.item_type = item_type # xác định loại item
        self.data = data # lưu dữ liệu của item
    def activate(self):		
        '''
        Kích hoạt item khi người chơi va chạm với item
        với item_type là silver: tăng 5 coins, diamond: tăng 50 coins, skull: tăng 1 mạng
...
class ParticleEffectSprite(AnimatedSprite):
    '''
    Khởi tạo lớp ParticleEffectSprite kế thừa từ lớp AnimatedSprite
    Lớp này tạo ra các đối tượng đồ họa là ParticleEffect trong trò chơi làm nhiệm vụ tạo hiệu ứng cho các vật thể trong trò chơi khi va chạm
    (các items khi người chơi nhặt, các vật thể khi bị bắn trúng)
    '''
    ...
    def animate(self, data_time):
        '''
        tạo animation cho ParticleEffect
        '''
        self.frame_index += self.animation_speed * data_time
